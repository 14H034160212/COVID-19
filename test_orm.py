...
def insert_user(session, values = None):  
    ...
    session.execute('INSERT INTO users (username, password) VALUES (:username, :password)',
        dict(username = new_name, password = new_password))
    row = session.execute('SELECT id from users where username = :username',
        dict(username = new_name)).fetchone()
    return row[0]

def insert_users(session, values):
    for value in values:
        session.execute('INSERT INTO users (username, password) VALUES (:username, :password)', 
            dict(username = value[0], password = value[1]))
...
def insert_article(session):    
    session.execute(
        ...
        dict(date = article_date.isoformat())
    )
...
def insert_article_tag_associations(session, article_key, tag_keys):
    ...
    for tag_key in tag_keys:
        session.execute(stmt, dict(article_id = article_key, tag_id = tag_key))
...
def insert_commented_article(session):
    ...
    session.execute(
        ...
        dict(user_id = user_key, article_id = article_key, timestamp_1 = timestamp_1, timestamp_2 = timestamp_2)
    )
...
def test_loading_of_users(session):
    ...
    expected = [
        model.User("Andrew", "1234"),
        model.User("Cindy", "999")
    ]
...
def test_loading_of_article(session):
    insert_article(session)
    expected = [make_article()]
    assert session.query(model.Article).all() == expected
...
def test_loading_of_tagged_article(session):
    ...
    article = session.query(model.Article).get(article_key)
    tags = [session.query(model.Tag).get(key) for key in tag_keys]
...
def test_loading_of_commented_article(session):
    ...
    rows = session.query(model.Article).all()
    article = rows[0]
...
def test_saving_of_comment(session):
    ...
    rows = session.query(model.Article).all()
    article = rows[0]
    user = session.query(model.User).filter(model.User._username == "Andrew").one()

    # Create a new Comment that is bidirectionally linked with the User and Article.
    comment_text = "Some comment text."
    comment = model.make_comment(comment_text, user, article)
    ...
    # commiting the addition of the Comment to the database.
    session.add(comment)
    session.commit()
...
def test_saving_tagged_article(session):
    ...
    model.make_tag_association(article, tag)
