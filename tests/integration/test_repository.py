from domain.model import User, Tag
...
def test_repository_can_retrieve_all_articles(populated_session):
    ...
    article_one = \
        [article for article in articles if article.title == "Coronavirus: First case of virus in New Zealand"][0]
    article_two = [article for article in articles if
                   article.title == "Coronavirus: Jacinda Ardern urges calm as panicked shoppers empty supermarket shelves"][
        0]
    article_three = [article for article in articles if
                     article.title == "Coronavirus: Rest homes and retirement villages plead for national aged care response plan"][
        0]
    ...
    article_two_comment_one = \
        [comment for comment in article_two.comments if comment.comment == "Loonies cleaning out the supermarkets"][0]
    ...
    assert article_one.is_tagged_by(Tag("News"))
    assert article_one.is_tagged_by(Tag("New Zealand"))
    assert article_two.is_tagged_by(Tag("News"))
...
def test_repository_can_retrieve_tags(populated_session):
    ...
    article_one_for_tag_one = [article for article in tag_one.tagged_articles if
                               article.title == "Coronavirus: First case of virus in New Zealand"][0]
    article_one_for_tag_two = [article for article in tag_two.tagged_articles if
                               article.title == "Coronavirus: First case of virus in New Zealand"][0]
    article_two_for_tag_one = [article for article in tag_one.tagged_articles if
                               article.title == "Coronavirus: Jacinda Ardern urges calm as panicked shoppers empty supermarket shelves"][
        0]
